...
<testsuite name="Appium Mobile UI Tests" tests="120" failures="5" errors="0" time="452.8">
  ...
  <testcase name="testBiometricAuthenticationSetup" classname="com.banking.tests.SettingsTests" time="10.8">
    <failure message="WebDriverException: An unknown server-side error occurred while processing the command. Original error: Could not verify fingerprint enrollment sensor state.">
  ...
  "vulnerability_details": [
    {
      ...
      "description": "The application is configured to allow cleartext HTTP traffic (android:usesCleartextTraffic=true) to all domains. This allows an attacker on the same network to perform Man-in-the-Middle (MitM) attacks and intercept sensitive data.",
    ...
    },
    {
      ...
      "description": "One or more database Content Providers are marked exported without requiring any permissions. Other apps on the device can query, modify, or corrupt user data.",
    ...
    },
    {
      ...
      "description": "A hardcoded AES symmetric encryption key was discovered in class com.banking.security.CryptoManager. An attacker can reverse-engineer the APK and decrypt local database files.",
    ...
    },
    {
      ...
      "description": "Biometric prompt validation is implemented using client-side callbacks without crypto object verification. An attacker can hook the app with Frida and spoof authentication success.",
      "remediation": "Pass a cryptographic key initialized with BiometricPrompt.CryptoObject to authenticate and decrypt data. This guarantees that cryptographic validation occurs on success."
    },
    {
      "severity": "high",
      "title": "Insecure Clipboard Usage",
      "section": "Data Leakage",
      "description": "Sensitive credentials and session tokens are copied to the system clipboard without setting them as sensitive content. Other background apps can access the clipboard history.",
      "remediation": "Avoid copying sensitive data to the clipboard. If necessary, use ClipDescription.EXTRA_IS_SENSITIVE to hide clipboard content from preview windows."
    },
    {
      "severity": "medium",
    ...
    },
    {
      ...
      "description": "Proguard/R8 code shrinking and obfuscation is disabled. This makes it trivial for attackers to reverse engineer the code and retrieve application logic.",
      "remediation": "Enable minifyEnabled true in build.gradle to obfuscate class and method names during release build compiling."
    },
    {
      "severity": "medium",
      ...
      "remediation": "Store application-specific transaction logs in internal private storage directories (context.getFilesDir())."
    },
    {
      "severity": "low",
      ...
      "remediation": "Configure Proguard/R8 rules to automatically strip logging statements in release builds, or check BuildConfig.DEBUG flags."
    }
  ]
}`;
